<x-app-layout>
    <div class="min-h-screen bg-gradient-to-br from-red-50 via-orange-50 to-yellow-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 p-4">
        <div class="max-w-7xl mx-auto">
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            <div class="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center backdrop-blur">
                                <i class="bi bi-exchange text-2xl"></i>
                            </div>
                            <div>
                                <h1 class="text-2xl font-bold">معاملات الموردين</h1>
                                <p class="text-red-100 text-sm">إدارة جميع معاملات الموردين</p>
                            </div>
                        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-sm text-gray-500 dark:text-gray-400">إجمالي الإيداعات</p>
                            <p class="text-xl font-bold text-green-600 dark:text-green-400">{{ number_format($totalDeposits, 2) }} ج.م</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-sm text-gray-500 dark:text-gray-400">إجمالي السحوبات</p>
                            <p class="text-xl font-bold text-red-600 dark:text-red-400">{{ number_format($totalWithdrawals, 2) }} ج.م</p>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 mb-6">
                <form action="{{ route('supplier.transactions.index') }}" method="GET" class="flex flex-col md:flex-row gap-4">
                    <div class="relative flex-1">
                        ...
                        <input 
                            ...
                            class="w-full pr-12 pl-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500 dark:bg-gray-700 dark:text-white transition-all">
                    </div>
                    <select name="filter" class="px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500 dark:bg-gray-700 dark:text-white">
                        <option value="">جميع المعاملات</option>
                        <option value="deposit" {{ request('filter') == 'deposit' ? 'selected' : '' }}>الإيداعات فقط</option>
                        <option value="withdrawal" {{ request('filter') == 'withdrawal' ? 'selected' : '' }}>السحوبات فقط</option>
                    </select>
                    <button 
                        type="submit" 
                        class="px-6 py-3 bg-gradient-to-r from-red-600 to-orange-600 text-white font-semibold rounded-xl hover:from-red-700 hover:to-orange-700 transition-all transform hover:scale-105 shadow-lg flex items-center gap-2">
                    ...
                    <table class="w-full">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                            @forelse($transactions as $transaction)
                                <tr class="hover:bg-red-50 dark:hover:bg-gray-700 transition-colors">
                                    <td class="p-4">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span class="font-medium">{{ $transaction->supplier->name }}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="p-4">
                                        <span class="px-2 py-1 rounded-full text-xs font-medium 
                                            @if($transaction->type == 'deposit') bg-green-100 dark:bg-green-900/20 text-green-700 dark:text-green-400
                                            @elseif($transaction->type == 'withdrawal') bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-400
                                            @else bg-gray-100 dark:bg-gray-900/20 text-gray-700 dark:text-gray-400 @endif">
                                            @if($transaction->type == 'deposit') إيداع
                                            @elseif($transaction->type == 'withdrawal') سحب
                                            @else تعديل @endif
                                        </span>
                                    </td>
                                    <td class="p-4 text-right font-bold">
                                        ...
                                        @else
                                            <span class="text-red-600 dark:text-red-400">-{{ number_format($transaction->amount, 2) }} ج.م</span>
                                        @endif
                                    </td>
                                ...
                                <tr>
                                    <td colspan="6" class="p-8 text-center text-gray-500 dark:text-gray-400">
                                        <div class="flex flex-col items-center gap-2">
                                            <i class="bi bi-exchange text-4xl"></i>
